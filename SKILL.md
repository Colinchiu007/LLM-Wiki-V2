...
description: |
  ...
  Adds on v1: knowledge lifecycle management, structured knowledge graph, hybrid search
  (BM25+Vector+Graph), automated governance, quality scoring, self-healing lint,
  privacy filtering, and contradiction detection.
  Triggers when:
    - User mentions "knowledge base v2", "wiki v2", "llm-wiki-v2"
    - User asks to "consolidate", "hybrid search", "knowledge graph traverse"
    - User asks to "supersede", "crystallize", "contradict"
    - User works with an initialized v2 knowledge base (ingest/query/lint/status)
  Do NOT trigger when user just says "summarize this article" — must be
  explicit knowledge-base intent.
metadata:
  hermes:
    tags:
      - knowledge-base
...
# llm-wiki-v2 — 个人知识库构建系统 v2

> 把碎片化的信息变成持续积累、互相链接、自我维护的知识库。
> 基于 Karpathy 原始 LLM Wiki + agentmemory 生产实践，新增生命周期管理、结构化图谱、混合搜索、自动化治理。

## 这个 skill 做什么

llm-wiki-v2 帮你构建一个**持续增长、自我维护的个人知识库**。它不是传统的笔记软件，而是一个让 AI 帮你维护的知识生态系统：

- 你给素材（链接、文件、文本），AI 提取核心知识并整理成互相链接的 wiki 页面
- 知识库随着每次使用变得越来越丰富，而不是每次重新开始
- **v2 新增**：知识有生命周期——置信度随时间衰减、随确认强化；过期知识自动降级
- **v2 新增**：结构化知识图谱——实体有类型、关系有语义，支持图遍历查询
- **v2 新增**：混合搜索——BM25 关键词 + Vector 语义 + Graph 结构，三种搜索融合
- **v2 新增**：自动化治理——隐私过滤、质量评分、自愈 lint、审计日志
...
**v2 核心升级**：
- **知识不是等权的**——置信度、来源数、时效性决定权重
- **知识不是永生的**——遗忘曲线让过时信息自动降级
- **知识不是孤立的**——结构化图谱让关联可遍历、可推理
- **知识不是手工维护的**——自动化钩子消除手动 bookkeeping
...
| **"巩固知识库"、"consolidate"** | → **consolidate** |
| **"搜索 XX"、"混合搜索"、"search"** | → **search** |
| **"XX 取代 YY"、"supersede"、"版本取代"** | → **supersede** |
| **"知识库升级"、"迁移 v2"、"migrate"** | → **migrate** |
| **"巩固知识库"、"consolidate"、"整理知识库"** | → **consolidate** |
| **"同步知识库"、"mesh-sync"、"推送/拉取"** | → **mesh-sync** |
| **"检测矛盾"、"矛盾扫描"、"contradiction"** | → **contradict** |
| **"自愈 lint"、"自动修复"、"self-heal"** | → **lint**（v2 增强模式） |
...
## 工作流 11：consolidate（知识巩固）

**触发条件**：
用户说"巩固知识库"、"consolidate"，或定时触发（建议每周一次）

**目标**：将工作记忆 → 情景记忆 → 语义记忆 → 程序性记忆，让知识库从"见过"升级为"理解"。
...
1. **识别高频访问内容**：
   - 扫描 `wiki/` 下所有 `.md` 文件的 frontmatter
   - 按 `access_count` 降序排列，取 top-10
   - 按 `confidence` 降序排列，取 top-10（高置信度 = 可巩固）
   - 合并去重，得到候选页面列表

2. **生成综合页面**：
   - 输入：候选页面的内容 + `purpose.md`
   - 输出：`wiki/synthesis/consolidated-{日期}.md`
   - 内容包含：跨页面的核心观点、模式、最佳实践
   - frontmatter 包含：
     ```yaml
     type: consolidation
     derived: true
     confidence: 0.85
     sources: [被巩固页面的路径列表]
     consolidated_from: [被巩固页面的路径列表]
     ```

3. **更新实体页**：
   - 将综合页面中的洞见提取到对应实体页
   - 标记 `last_confirmed` 为今天（强化置信度）
   - 运行：
     ```bash
     python ${SKILL_DIR}/lib/confidence.py reinforce "<entity_page_path>"
     ```

4. **归档低频内容**：
   - `access_count < 3` 且 `last_accessed > 180 天` → 移至 `wiki/archive/`
   - 移至 archive 的页面在原位置保留一个引用：
     ```markdown
     > 本页已归档至 [[archive/原页面名]]，因为超过 180 天未被访问。
     ```

5. **更新索引和日志**：
   - index.md 的"综合分析"分类下添加巩固报告
   - log.md 追加：`## {日期} consolidate | 知识巩固`
...
6. **向用户展示结果**：
   ...
   **zh**：
   ...
   生成综合报告：wiki/synthesis/consolidated-{日期}.md

   归档低频内容：
   - [[页面X]] → archive/（180 天未访问）
   ```
   （英文版按「输出语言规则」生成，结构相同。）
...
**触发条件**：
用户说"搜索 XX"、"混合搜索"、"search"，或在 query 工作流中需要更精确的检索。

### 前置检查

执行**通用前置检查**。检查搜索索引是否已构建：
- 如果 `$WIKI_ROOT/.wiki-search.db` 不存在 → 运行 `index-bm25.sh` 构建
- 如果 `$WIKI_ROOT/.chroma/` 不存在 → 提示用户需要先运行索引构建
...
| `vector` | Chroma + embeddings | 语义相似度 |
| `graph` | graph-data.json 遍历 | 实体关联发现 |
| `hybrid`（默认） | RRF 融合上述三种 | 综合查询 |

### 步骤

1. **实体识别**：
   - 从查询中提取实体（人名、项目名、技术名等）
   - 在 `graph-data.json` 中查找对应节点

2. **三路并行搜索**：

   **BM25**：
   ```bash
   bash ${SKILL_DIR}/scripts/search-bm25.sh "$WIKI_ROOT" "<query>"
   ```

   **Vector**（如果已构建）：
   ```bash
   python ${SKILL_DIR}/lib/search.py vector-search "$WIKI_ROOT" "<query>" --top-k 20
   ```

   **Graph**（如果提取到实体）：
   ```bash
   python ${SKILL_DIR}/lib/search.py graph-search "$WIKI_ROOT" "<entity>" --max-depth 2
   ```

3. **RRF 融合**：
   ```bash
   python ${SKILL_DIR}/lib/search.py fuse \
     --bm25-results /tmp/bm25-results.json \
     --vector-results /tmp/vector-results.json \
     --graph-results /tmp/graph-results.json \
     --top-k 10
   ```

4. **置信度重排序**：
   - 读取融合结果的 frontmatter
   - 按 `confidence × retrieval_priority` 重排序
   - 返回 top-5 页面给 LLM 生成回答
...
5. **输出结果**：
   ...
   **zh**：
   ...
   来源：BM25 {N} 条 + Vector {M} 条 + Graph {K} 条 → 融合 top-10

   Top 结果：
   1. [[页面A]] (confidence: 0.9, bm25#2, vector#1, graph#3)
   2. [[页面B]] (confidence: 0.8, bm25#1, vector#4)
   3. [[页面C]] (confidence: 0.7, vector#2)
   ```
   （英文版按「输出语言规则」生成，结构相同。）
...
```bash
# BM25 索引（必需，快速）
bash ${SKILL_DIR}/scripts/index-bm25.sh "$WIKI_ROOT"

# Vector 索引（可选，需要 chromadb）
pip install chromadb sentence-transformers
python ${SKILL_DIR}/lib/search.py index-vector "$WIKI_ROOT"
```

建议在 `init` 工作流完成后自动运行 BM25 索引构建。

---

## 工作流 13：supersede（版本取代）

**触发条件**：
用户说"XX 取代 YY"、"supersede"、"版本取代"，或在 ingest/lint 中检测到矛盾信息。

### 步骤

1. **确认取代关系**：
   - 用户指定旧页面和新页面（或在 lint 中自动检测到矛盾）
   - 展示两个页面的核心声明，让用户确认取代方向

2. **执行取代**：
   ```bash
   bash ${SKILL_DIR}/scripts/supersede.sh "$WIKI_ROOT" "<old_page>" "<new_page>"
   ```

   脚本执行：
   - 旧页面 frontmatter：`status: superseded`，`superseded_by: [<new_page>]`
   - 新页面正文顶部添加更新说明
   - 可选：旧页面移至 `wiki/archive/`

3. **更新引用**：
   - 扫描所有引用旧页面的 `[[链接]]`
   - 在链接旁添加标注：`[[旧页面|已归档]] → [[新页面]]`

4. **更新图谱**：
   - `graph-data.json` 中旧节点的边迁移到新节点
   - 旧节点标记为 `deprecated`

5. **审计日志**：
   ```bash
   bash ${SKILL_DIR}/scripts/audit-logger.sh "$WIKI_ROOT" supersede "<old_page> → <new_page>" "用户确认"
   ```
...
## 工作流 14：migrate（v1 → v2 迁移）

**触发条件**：
用户说"知识库升级"、"迁移 v2"、"migrate"

### 步骤

1. **确认迁移源**：
   - 检测当前知识库是否为 v1 格式（页面无 frontmatter 或 frontmatter 缺少 `confidence` 字段）
   - 提示用户：迁移将为所有页面补充 v2 元数据（frontmatter），不会删除或修改正文内容
...
3. **补充 frontmatter**：
   - 遍历 `wiki/{entities,topics,sources,synthesis,queries}/` 下所有 `.md` 文件
   - 对无 frontmatter 的页面：
     - 从 `.wiki-cache.json` 读取 source 信息
     - 从 `<!-- confidence: XXX -->` 注释提取置信度（默认 0.5）
     - 生成标准 v2 frontmatter 并写入文件头部
...
6. **验证**：
   - 统计已迁移页面数和未迁移页面数
   - 随机抽查 5 个页面，确认 frontmatter 格式正确
...
7. **向用户报告**：
   ...
   **zh**：
   ...
   已迁移：{N} 个页面（补充了 v2 frontmatter）
   已跳过：{M} 个页面（已有 v2 frontmatter）

   新增基础设施：
   - BM25 搜索索引：.wiki-search.db
   - 知识图谱数据：wiki/graph-data.json（已重建）
...
## 工作流 15：contradict（矛盾检测）

**触发条件**：
用户说"检测矛盾"、"矛盾扫描"、"contradiction"，或在 lint 中检测到矛盾信息。

### 前置检查

执行**通用前置检查**。如果没有可用知识库，提示用户先初始化。

### 步骤

1. **运行矛盾扫描**：
   ```bash
   python ${SKILL_DIR}/lib/contradiction.py scan "$WIKI_ROOT" --threshold 0.7
   ```

2. **审查结果**：
   - 如果发现矛盾 → 展示给用户，建议处理方式（supersede 或修正）
   - 如果无矛盾 → 报告知识库状态健康

3. **处理矛盾**（用户确认后）：
   - 矛盾类型 `negation_conflict` → 建议走 supersede 工作流
   - 标记矛盾双方为 `contradicts` 关系
   ```bash
   python ${SKILL_DIR}/lib/entities.py add-edge "$WIKI_ROOT" \
     --source "<page1>" --target "<page2>" --type contradicts
   ```

4. **审计日志**：
   ```bash
   bash ${SKILL_DIR}/scripts/audit-logger.sh "$WIKI_ROOT" contradict "<page1> ↔ <page2>" "矛盾检测"
   ```
...
当触发词包含"自愈"、"自动修复"、"self-heal"时，lint 工作流在 Step 0 之后增加自愈步骤：

```bash
bash ${SKILL_DIR}/scripts/self-heal.sh "$WIKI_ROOT" [--dry-run]
```

自愈步骤包括：
1. **置信度衰减检查** → 自动将 active → stale / deprecated
2. **质量评分** → 标记低质量页面（quality < 0.4）
3. **Frontmatter 补充** → 为缺失 v2 frontmatter 的页面补充默认值
4. **断链检测** → 列出指向不存在页面的 [[wikilinks]]
5. **孤立页面检测** → 找出无入链的页面

`--dry-run` 模式只报告不修改。默认使用 `--dry-run`，用户确认后再执行实际修复。

---

## 工作流 16：consolidate（知识巩固）

**触发条件**：
用户说"巩固知识库"、"consolidate"、"整理一下知识库"。
...
1. **识别高频/高置信度页面**：
   ```bash
   python ${SKILL_DIR}/lib/consolidate_ops.py identify "$WIKI_ROOT"
   ```

2. **强化高置信度页面**（reinforce）：
   ```bash
   python ${SKILL_DIR}/lib/consolidate_ops.py reinforce "$WIKI_ROOT" [--dry-run]
   ```
   置信度 ≥ 0.7 的页面会自动 reinforce（提高置信度、更新 access_count）。

3. **归档低频内容**：
   ```bash
   python ${SKILL_DIR}/lib/consolidate_ops.py archive "$WIKI_ROOT" [--dry-run]
   ```
   条件：access_count < 3 且 last_accessed > 180 天。
   归档后原位置留下引用，指向 `archive/` 目录中的副本。

4. **审计日志**：
   ```bash
   bash ${SKILL_DIR}/scripts/audit-logger.sh "$WIKI_ROOT" consolidate "知识巩固" "系统自动"
   ```

5. **向用户报告**：
   - 强化了 N 个高置信度页面
   - 归档了 N 个低频页面（dry-run 先展示）
   - 建议用户确认后再执行实际归档

---

## 工作流 17：mesh-sync（多 Agent 同步）

**触发条件**：
用户说"同步知识库"、"mesh-sync"、"推送/拉取知识库"。
...
1. **初始化 Git 仓库**（首次）：
   ```bash
   bash ${SKILL_DIR}/scripts/mesh-sync.sh "$WIKI_ROOT" --init
   ```

2. **推送到远程**：
   ```bash
   bash ${SKILL_DIR}/scripts/mesh-sync.sh "$WIKI_ROOT" --push
   ```

3. **拉取远程更新**：
   ```bash
   bash ${SKILL_DIR}/scripts/mesh-sync.sh "$WIKI_ROOT" --pull
   ```

4. **查看状态**：
   ```bash
   bash ${SKILL_DIR}/scripts/mesh-sync.sh "$WIKI_ROOT" --status
   ```
...
llm-wiki-v2 支持通过 OpenClaw cron 和 heartbeat 机制实现自动化：

| 任务 | 建议 cron | 说明 |
|------|----------|------|
| 知识巩固 + 自愈 lint | `0 2 * * 0`（每周日凌晨2点） | consolidate + self-heal |
| raw/ 目录扫描 | `*/30 * * * *`（每30分钟） | scan-raw.sh 检查新素材 |
| 矛盾检测 | `0 3 * * *`（每天凌晨3点） | contradiction.py scan |

注册方式：
```bash
# 需要在 OpenClaw CLI 中手动执行
openclaw cron add --schedule '0 2 * * 0' --task '对知识库执行巩固和自愈 lint'
openclaw cron add --schedule '*/30 * * * *' --task '检查 raw/ 目录是否有新素材'
openclaw cron add --schedule '0 3 * * *' --task '运行矛盾检测扫描'
```

或者将任务添加到 `HEARTBEAT.md` 中由 agent 在心跳检查时轮询执行。

### Session 生命周期钩子

- **Session 开始**：自动加载 `purpose.md` 和最近访问的 5 个页面（通过 access_count 排序）
- **Session 结束**：检查当前会话是否包含有价值的知识内容，如果有则建议 crystallize

---

## v2 Frontmatter 规范

所有 v2 页面必须包含以下 frontmatter 字段：

```yaml
---
# 必需字段
confidence: 0.85          # 置信度 0-1，由 LLM 评估
sources:                   # 支持该声明的来源列表
  - raw/articles/2026-01-15-xxx.md
created: 2026-06-02       # 创建日期
last_accessed: 2026-06-02 # 最后访问日期
access_count: 3            # 被访问次数
status: active             # active | stale | deprecated | superseded

# 可选字段
authority: 0.7             # 来源权威度（用户可手动标注，默认 0.5）
superseded_by: []          # 被哪些页面取代（status=superseded 时填充）
quality: 0.8              # 质量评分（0-1）
tags: []                   # 标签
---
```

**向后兼容**：旧页面无 frontmatter 时，默认值为 `confidence: 0.5`, `status: active`, `access_count: 0`。
...
**生命周期规则**：
- 超过 90 天未访问 → `status: stale`
- 超过 180 天未访问 → `status: deprecated`
- 被 supersede → `status: superseded`
...
| 关系类型 | 含义 | 方向性 | 示例 |
|----------|------|--------|------|
| `uses` | A 使用 B | 单向 | "项目 X 使用 Redis" |
| `depends_on` | A 依赖 B | 单向 | "前端依赖 React" |
| `causes` | A 导致 B | 单向 | "配置错误导致宕机" |
| `contradicts` | A 与 B 矛盾 | 双向 | "来源 A 说 X，来源 B 说 Y" |
| `supersedes` | A 取代 B | 单向 | "新方案取代旧方案" |
| `related_to` | A 与 B 相关 | 双向 | 通用关联 |
| `part_of` | A 是 B 的一部分 | 单向 | "缓存模块是系统的一部分" |
| `implemented_by` | A 由 B 实现 | 单向 | "认证机制由 OAuth 实现" |

---

## v2 审计日志

所有写操作记录到 `wiki/.audit.log`，格式：

```
2026-06-02T19:02:33Z | ingest | wiki/sources/2026-06-02-xxx.md | from raw/xxx.md | agent:agent-c693c441
2026-06-02T19:15:00Z | edit | wiki/entities/XXX.md | updated by LLM (confidence:0.85) | agent:agent-c693c441
2026-06-02T19:30:00Z | supersede | wiki/entities/Old.md → wiki/entities/New.md | 用户确认 | agent:agent-c693c441
```
...
ingest 工作流在"隐私自查提示"之后，自动运行敏感信息扫描：

```bash
python ${SKILL_DIR}/lib/privacy_filter.py scan "<raw_content_file>"
```

如果检测到敏感信息（API Key、密码、手机号、身份证号）：
1. 提示用户确认是否自动过滤（替换为 `[REDACTED]`）
2. 用户同意 → 自动过滤后继续 ingest
3. 用户拒绝 → 终止 ingest
...
```
my-wiki/
├── .wiki-schema.md          # 知识库 Schema（增强：新增关系类型、置信度规则）
├── .wiki-cache.json         # 缓存（不变）
├── .wiki-search.db          # BM25 搜索数据库（v2 新增）
├── .audit.log               # 审计日志（v2 新增）
├── purpose.md               # 研究目标（不变）
├── index.md                 # 索引（不变）
├── log.md                   # 日志（不变）
├── raw/                     # 原始素材（不变）
├── wiki/                    # Wiki 页面（增强：含 v2 frontmatter）
│   ├── entities/            # 实体页
│   ├── topics/              # 主题页
│   ├── sources/             # 素材摘要
│   ├── synthesis/           # 综合分析
│   ├── queries/             # 查询记录
│   ├── archive/             # 归档页面（v2 新增）
│   ├── graph-data.json      # 知识图谱数据（增强：结构化关系）
│   └── knowledge-graph.html # 交互式图谱（不变）
└── .chroma/                 # Vector 搜索数据库（v2 新增，gitignore）
```
