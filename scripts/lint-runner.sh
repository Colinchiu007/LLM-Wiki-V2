...
index_has_entry() {
  local entry="$1"
  # Use cached index links if available
  if [ -n "${_INDEX_CACHE:-}" ] && [ -f "$_INDEX_CACHE" ]; then
    grep -Fxq "$entry" "$_INDEX_CACHE"
  else
    grep -Fq "[[$entry]]" "$INDEX_FILE" 2>/dev/null
  fi
}
...
echo "--- 断链（被链接但不存在的页面） ---"
_TMP_BROKEN=$(mktemp)
_EXTRACT_SCRIPT="$(cd "$(dirname "$0")" && pwd)/extract-wiki-links.js"
_ALL_LINKS=$(mktemp)
if [ -f "$_EXTRACT_SCRIPT" ] && command -v node >/dev/null 2>&1; then
  node "$_EXTRACT_SCRIPT" "$WIKI_DIR" 2>/dev/null > "$_ALL_LINKS"
else
  grep -rohF '[[[' "$WIKI_DIR" 2>/dev/null > /dev/null
  # Fallback: perl (MSYS2 grep -E breaks on UTF-8)
  if command -v perl >/dev/null 2>&1; then
    perl -Mutf8 -nle 'while(/\[\[([^\]]+)\]\]/g){my$n=$1;$n=~s/\|.*//;print $n}' "$WIKI_DIR"/*.md "$WIKI_DIR"/**/*.md 2>/dev/null | sort -u > "$_ALL_LINKS"
  fi
fi
sort -u "$_ALL_LINKS" | \
...
_INDEX_LINKS=$(mktemp)
if [ -f "$_EXTRACT_SCRIPT" ] && command -v node >/dev/null 2>&1; then
  node "$_EXTRACT_SCRIPT" "$WIKI_DIR" --index "$INDEX_FILE" 2>/dev/null > "$_INDEX_LINKS"
else
  grep -ohE '\[\[[^]]+\]\]' "$INDEX_FILE" 2>/dev/null | \
    sed -e 's/\[\[//g' -e 's/\]\]//g' -e 's/|.*//' > "$_INDEX_LINKS"
fi
sort -u "$_INDEX_LINKS" | \
...
rm -f "$_TMP_MISSING" "$_INDEX_LINKS"
echo ""
...
# Build index cache once for fast lookup (MSYS2-safe: use extract-wiki-links.js)
_EXTRACT_SCRIPT="$(cd "$(dirname "$0")" && pwd)/extract-wiki-links.js"
_INDEX_CACHE=$(mktemp)
if [ -f "$_EXTRACT_SCRIPT" ] && command -v node >/dev/null 2>&1; then
  node "$_EXTRACT_SCRIPT" "$WIKI_DIR" --index "$INDEX_FILE" 2>/dev/null > "$_INDEX_CACHE"
  export _INDEX_CACHE
else
  _INDEX_CACHE=""
fi
...
rm -f "$_TMP_UNLISTED" "$_INDEX_CACHE"
unset _INDEX_CACHE
echo ""
